...
export default function Learning() {
  ...
  if (loading) {
    return (
      <div className="learning-page min-h-screen bg-slate-950 p-6 lg:p-10 space-y-8">
  ...
  if (error) {
    return (
      <div className="learning-page min-h-screen bg-slate-950 flex items-center justify-center">
  ...
  return (
    <div className="learning-page min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 pb-20">
      {/* Hero Header */}
    <div className="bg-gradient-to-br from-slate-900 via-slate-900/95 to-slate-950 border-b border-slate-800 pt-12 pb-12 px-6 lg:px-10 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
                  <BookOpen className="text-emerald-300" size={28} />
                </div>
      ...
      <div className="learning-content max-w-7xl mx-auto px-6 lg:px-10 mt-0 pt-8 lg:pt-10 space-y-12">
        ...
        {(stats.totalVideos > 0 ||
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-800 hover:shadow-md transition-all group">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-sky-500/10 rounded-lg group-hover:bg-sky-500/20 transition-colors">
                  <Play className="text-sky-300" size={20} />
                </div>
                <div>
                  <p className="text-2xl font-bold text-slate-100">
                    {stats.completedVideos}
                  </p>
                  <p className="text-xs text-slate-400">Videos Watched</p>
                </div>
              </div>
            </div>
            <div className="bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-800 hover:shadow-md transition-all group">
        ...
        {history.continueWatching.length > 0 ? (
          <section>
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-emerald-500/10 rounded-lg border border-emerald-500/20">
                <Clock className="text-emerald-300" size={22} />
              </div>
            ...
            <div className="flex gap-6 overflow-x-auto pb-6 snap-x custom-scrollbar scroll-smooth">
              {history.continueWatching.map((item, index) => (
                <Link
                  ...
                  className="snap-start shrink-0 w-72 group bg-slate-900 rounded-xl shadow-sm border border-slate-800 overflow-hidden hover:shadow-xl hover:scale-[1.02] transition-all duration-300"
                ...
                >
                  <div className="relative aspect-video bg-slate-800 overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="w-14 h-14 bg-slate-100/95 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
                  ...
                  <div className="p-5">
                    <h3 className="font-semibold text-slate-100 line-clamp-2 mb-2 group-hover:text-emerald-400 transition-colors">
          ...
          <section>
            ...
            <div className="bg-slate-900 rounded-2xl p-12 border border-dashed border-slate-700 text-center">
              <div className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-emerald-500/20">
                <Clock className="text-emerald-300" size={32} />
              </div>
        ...
        {history.smartReview.length > 0 ? (
          <section>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-slate-100">Smart Review</h2>
              <span className="ml-auto text-sm text-slate-400">
                {history.smartReview.length} topic
                {history.smartReview.length !== 1 ? "s" : ""} to review
              </span>
            </div>
            <div className="bg-gradient-to-br from-amber-500/10 to-orange-500/10 rounded-2xl p-8 border border-amber-500/20 shadow-sm">
                ...
                {history.smartReview.map((item, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-slate-900 p-5 rounded-xl border border-amber-500/20 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02] group"
                      style={{ animationDelay: `${index * 50}ms` }}
                    >
                      <div className="flex items-start gap-4">
                        <div className="shrink-0 w-14 h-14 bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-xl flex items-center justify-center text-amber-200 font-bold text-lg shadow-sm group-hover:scale-110 transition-transform">
                          {score}%
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-semibold text-slate-100 line-clamp-2 mb-2 group-hover:text-amber-300 transition-colors">
                            {item.title}
                          </h4>
                          ...
                          <Link
                            ...
                            className="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:text-emerald-700 group-hover:gap-2 transition-all"
                          >
        ...
        <section>
          <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-slate-100">My Playlists</h2>
              {playlists.length > 0 && (
            ...
            {playlists.length > 4 && (
              <Link
                to="/playlist"
                className="text-sm font-medium text-emerald-300 hover:text-emerald-200 flex items-center gap-1 hover:gap-2 transition-all"
              >
          ...
          {playlists.length === 0 ? (
            <div className="text-center py-16 bg-slate-900 rounded-2xl border border-dashed border-slate-700">
              <div className="w-20 h-20 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-500/20">
                <BookOpen className="text-emerald-300" size={40} />
              </div>
              <h3 className="text-lg font-semibold text-slate-100 mb-2">
                No playlists yet
              </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {playlists.slice(0, 4).map((playlist, index) => (
                <Link
                  ...
                  className="group bg-slate-900 rounded-xl shadow-sm border border-slate-800 overflow-hidden hover:shadow-xl hover:scale-[1.02] transition-all duration-300"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                    ...
                    {playlist.videos?.[0]?.videoId ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-slate-500 group-hover:text-emerald-400 transition-colors">
                        <BookOpen size={40} />
                      </div>
                    )}
