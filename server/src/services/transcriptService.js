...
function isSafeInvidiousHost(host) {
  if (!host || typeof host !== "string") return false;
  const normalized = host.trim().toLowerCase();

  // Allow only plain hostnames (no scheme, no path, no query, no spaces).
  if (!/^[a-z0-9.-]+$/.test(normalized)) return false;

  // Block obvious local/internal targets.
  if (
  ...
  try {
    ...
    const instances = data
      .filter(item => {
        ...
        return meta.type === "https" && isSafeInvidiousHost(domain);
      })
      .map(item => item[0])
...
async function fetchInvidiousTranscript(videoId) {
  ...
  for (const domain of uniqueInstances) {
    ...
    try {
      ...
      const trackRes = await fetchWithTimeout(trackUrl, {}, 8000);
